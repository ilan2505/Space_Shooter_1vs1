...
def draw_window(red, yellow, red_bullets, yellow_bullets,red_health,yellow_health):
    WIN.blit(SPACE,(0,0))
    # WIN.fill(WHITE)
    # pygame.draw.rect(WIN, pygame.Color(50, 50, 50), r)
...
def yellow_handle_movment(key_pressed, yellow):
    if key_pressed[pygame.K_a] and yellow.x - VEL > 0:  # left
        yellow.x -= VEL
    if key_pressed[pygame.K_d] and yellow.x + VEL + yellow.width - 1 < BORDER.x:  # right
        yellow.x += VEL
...
def red_handle_movment(key_pressed, red):
    if key_pressed[pygame.K_LEFT] and red.x - VEL > BORDER.x:  # left
        red.x -= VEL
